...
parser.add_argument("-o" "--output_file", action="store", type=str, help="name of output file", dest="output_file", required=True)
parser.add_argument("-n", "--ncores", action="store", type=int, help="Number of cores to use", dest="ncores", required=True)
args = parser.parse_args()

if __name__ == '__main__':
	print(f"Running MD with {args.ncores} threads")
	...
	simulation = Simulation(pdb.topology, system, integrator, platform)#, platformProperties={'Threads':str(args.ncores)})
